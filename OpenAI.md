...
- Cherry-pick base in this repo: `a061098` (`refactor: remove unnecessary use of itertools cargo dep (#1016)`)
...
|---|-------|----------|-----------------|---------|
| 1 | `8634d49` | `f123138` | — | fix(sdist): prefer setup.py when pyproject metadata is incomplete |
| 2 | `c4540ce` | `9c1e4de` | — | fix(uv): preserve wheel metadata and expose dist_info |
| 3 | `1d1e0b9` | `5d81044` | — | fix(uv): wrap sdist compilers to strip unsupported debug flags |
| 4 | `c8edc06` | `ecd1eb6` | [#1010](https://github.com/aspect-build/rules_py/pull/1010) (open, upstreaming) | fix(py): reset Python flags on data edges |
| 5 | `0fd3377` | `8d8371f` | — | fix(uv): require an explicit venv for target compatibility |

**Note:** Row 3 (`1d1e0b9`, "wrap sdist compilers to strip unsupported debug flags") does not yet have a dedicated upstream PR. Now that [#1004](https://github.com/aspect-build/rules_py/pull/1004) has merged (`edf46ec`), the new `pep517_native_whl(toolchains = ..., env = ...)` plumbing exposes a way to swap in a different CC toolchain via `uv.override_package(toolchains = ...)` — which could host a filter-aware variant and obviate the per-action Python wrappers that currently strip `-fdebug-default-version=4` from `CC`/`CXX`/`MPICC`. But #1004 itself doesn't ship that filter and isn't the canonical replacement. Keep row 3 until a dedicated CC-wrapping PR lands.
...
- `d0c3156` — fix(uv): add explicit default targets to wheel select — [aspect-build/rules_py#995](https://github.com/aspect-build/rules_py/pull/995) (merged 2026-05-11 as `d4b8109`)
  ...
  - One related conflict was resolved during that rebase in `uv/private/whl_install/defs.bzl`, where `c4540ce` ("preserve wheel metadata and expose dist_info") also touched `select_chain`'s empty-arms path: kept main's version (it supersedes the patch's contribution to this file).
...
- `738a7ca` — fix(uv): always keep sdist fallbacks when sources exist — [aspect-build/rules_py#998](https://github.com/aspect-build/rules_py/pull/998) (merged 2026-05-11 as `549193d`)
  ...
  - One related conflict was resolved during that rebase in `uv/private/extension/defs.bzl`, where `c4540ce` ("preserve wheel metadata and expose dist_info") had: (a) added the `_merge_build_deps` helper and rewritten the `lock_build_deps`/`build_deps` merge to use it (rather than the post-hoc `sets.to_list(sets.make(...))` dedup), and (b) preserved a now-stale FIXME comment about old `setup.py`-style packages. Took main's choice on both: removed the redundant `sets.to_list(sets.make(...))` dedup line (the `_merge_build_deps` rewrite already dedups) and dropped the FIXME comment block (matching `549193d`).
...
- **`0fd3377`** (`8d8371f`, "require an explicit venv for target compatibility") — error message reworded for the `venv` → `dep_group` rename. The CLI flag is now `--@aspect_rules_py//uv/private/constraints/dep_group:dep_group=<name>`, and the target attribute is `dep_group = ...`.
